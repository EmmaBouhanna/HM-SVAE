...
class VonMisesFisher(torch.distributions.Distribution):
    ...
    def __while_loop(self, b, a, d, shape, k=20, eps=1e-20):
        ...
        while bool_mask.sum() != 0:
            ...
            # t = (2 * a * b) / (1 - (1 - b) * e_)
            accept = torch.distributions.Beta(con1, con2).cdf(e)/torch.distributions.Beta(con1, con2).cdf(e_)
            accept = accept*torch.exp(w_*self.scale)*(1 - w_**2)**(.5*(self.__m -3))/(torch.exp(w*self.scale)*(1- w**2)**(.5*(self.__m - 3)))
            accept = (u < accept)
